...
All notable changes to Telemachus will be documented in this file. Entries
before the Telemachus fork retain their historical SideScreen names and links.
...
### Added
- Telemachus Mac and Android application identity and onboarding.
- A distinct Signal Gate icon system across macOS, Android, the website, and
  repository documentation.
- Prominent SideScreen fork attribution in first-run UI, About credits, the
  Android disconnected screen, website lineage section, and packaged notices.
- Existing-display streaming in addition to virtual extended displays.
- USB reconnect self-healing and Android client auto-launch.
- Transport self-tests, Android protocol tests, and release verification.
- Security, privacy, protocol, threat-model, support, and contributor policies.
- ZXing-based local QR decoding in place of the proprietary ML Kit SDK.

### Changed
- Stream queues favor the newest safe frame instead of accumulating latency.
- Release builds require project-controlled Android signing credentials.
- macOS release packaging distinguishes ad-hoc source builds from notarized
  Developer ID releases, marks unsigned releases as prereleases, and publishes
  SHA-256 checksums.
- Website performance language now distinguishes software diagnostics from
  unmeasured glass-to-glass latency.

### Security
- Wireless mode is explicitly documented as authenticated but unencrypted and
  suitable only for a trusted LAN.
- Pairing credentials are Android Keystore-encrypted and excluded from backup;
  the Mac token is stored in the non-synchronizing login Keychain.
- Connection admission, timeout, and token-reset lifecycles are hardened.
...
[Unreleased]: https://github.com/aaditagrawal/telemachus/compare/0.12.0...HEAD
[0.12.0]: https://github.com/aaditagrawal/telemachus/releases/tag/0.12.0
